...
printVersionHeader("AddParseVar $version");
if ($help) { printHelp("addParseVar"); }
	...
	while (<FILE>) {
	    ...
	    my $id = $1;
	    my $target = removeNITEID($2);
	    
	    if (!$cases{$id}) {
		if ($verbose) {
		    # die if there is no item ID in the input/output file that corresponds 
		    # to the the ID currently being processed                                                
